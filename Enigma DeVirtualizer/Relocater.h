...
{
public:
	bool Relocate(std::map<DWORD,DWORD> &RelocMapKey, std::map<DWORD,DWORD> &RelocAddrKey, DWORD FileMapVA);
	bool RelocateAddr(std::map<DWORD,DWORD> &RelocMapAddr, DWORD FileMapVA, DWORD typesPtr);
	bool RelocateKey(std::map<DWORD,DWORD> &RelocMapKey, DWORD typesPtr);
	bool ConvertKeyRefs(std::map<DWORD,DWORD> &RelocMap, std::map<int, DWORD> &KeyMap);
	bool FixEntries(std::map<DWORD,refs> &log_list, std::map<DWORD,DWORD> &RelocMapAddr, std::map<int,DWORD> &KeyMap, DWORD FileMapVA);
protected:
private:
};